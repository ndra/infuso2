<?

/**
 * Модель шаблона письма
 **/
class user_mail_template extends reflex {


    

public static function reflex_table() {return array (
  'name' => 'user_mail_template',
  'fields' => 
  array (
    0 => 
    array (
      'name' => 'id',
      'type' => 'jft7-kef8-ccd6-kg85-iueh',
      'editable' => '1',
      'id' => 'ylixmgs7kvsopvr7fv2dcl0jpv2tye',
    ),
    1 => 
    array (
      'name' => 'code',
      'type' => 'v324-89xr-24nk-0z30-r243',
      'editable' => 2,
      'id' => 'roc52ocuhjwlhdyab34lhnk5in4a2n',
      'label' => 'Код',
      'indexEnabled' => 0,
    ),
    2 => 
    array (
      'editable' => 1,
      'id' => 'fctfdw0a3bd2wjphej7lv1bavmri7c',
      'name' => 'disable',
      'type' => 'fsxp-lhdw-ghof-1rnk-5bqp',
      'label' => 'Отключить отправку всех сообщений с текущим кодом',
      'group' => '',
      'default' => '',
      'indexEnabled' => 0,
      'help' => '',
    ),
    3 => 
    array (
      'name' => 'enable',
      'type' => 'fsxp-lhdw-ghof-1rnk-5bqp',
      'editable' => '1',
      'id' => '07qczk0y45blm7qzemxplptqx1977z',
      'label' => 'Включить',
      'indexEnabled' => 0,
    ),
    4 => 
    array (
      'name' => 'from',
      'type' => 'v324-89xr-24nk-0z30-r243',
      'editable' => '1',
      'id' => '0dfubdwgro41i349s3w9r3carqk503',
      'label' => 'От',
      'indexEnabled' => 0,
    ),
    5 => 
    array (
      'name' => 'subject',
      'type' => 'v324-89xr-24nk-0z30-r243',
      'editable' => '1',
      'id' => 'r7f5bof9z78527cgsokghn41rxmgsx',
      'label' => 'Тема',
      'indexEnabled' => 0,
    ),
    6 => 
    array (
      'name' => 'message',
      'type' => 'kbd4-xo34-tnb3-4nxl-cmhu',
      'editable' => '1',
      'id' => '45rxclrjy5z7w9so4gsnp5bdmlbt85',
      'label' => 'Сообщение',
      'indexEnabled' => 0,
    ),
    7 => 
    array (
      'name' => 'layout',
      'type' => 'kbd4-xo34-tnb3-4nxl-cmhu',
      'editable' => '1',
      'id' => 'ls805u3sozlwytayc02oqooesaeltw',
      'indexEnabled' => 0,
      'label' => 'Каркас письма',
      'group' => '',
      'default' => '',
      'help' => '',
    ),
    8 => 
    array (
      'editable' => 2,
      'id' => 'ltf6zlzdhxytjevwkivfblb63te7rr',
      'name' => 'params',
      'type' => 'kbd4-xo34-tnb3-4nxl-cmhu',
      'label' => 'Параметры',
      'group' => '',
      'default' => '',
      'indexEnabled' => 0,
      'help' => '',
    ),
  ),
  'indexes' => 
  array (
  ),
  'fieldGroups' => 
  array (
    0 => 
    array (
      'name' => NULL,
      'title' => NULL,
    ),
  ),
);}

/**
     * Возвращает коллекцию всех элементов
     **/
    public static function all() {
        return reflex::get(get_class())->asc("code");
    }

    /**
     * @return Возвращает элемент по id
     **/
    public static function get($data) {
        return reflex::get(get_class(),$data);
    }
    
    public function reflex_root() {
        return array(
            self::all()->param("tab","user")->title("Шаблоны писем"),
        );
    }
    
    public function reflex_title() {
        return $this->data("code");
    }
    
    public function dataWrappers() {
        return array(
            "disable" => "mixed/data",
            "code" => "mixed/data",
            "enable" => "mixed/data",
            "subject" => "mixed/data",
        );
    }
    
    
}
